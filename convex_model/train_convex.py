...
from data import PianoToGuitar, A_Matrices
...
# Load datasets
dataset_train = A_Matrices(args.data, 'train', device)
...
print('Num test samples:', len(dataset_valid))
...
# Create model
model = model.GradDescent(dataset_train.matrices, device)
model.to(device)
...
for epoch in range(epochs):
    ...
    for samples_piano, samples_guitar in dataloader_valid:
        ...
        loss = mae_loss(piano_transformed, samples_guitar)   
    ...
    print('Train Loss:',train_loss / (len(dataloader_valid) / batch_size) )
    ...
    piano_transformed = piano_transformed.cpu().numpy()
    samples_piano = samples_piano.cpu().numpy()
    samples_guitar = samples_guitar.cpu().numpy()
    #print(samples_piano[0].shape, piano_transformed[0].shape, samples_guitar[0].shape)
    sf.write('input.wav', samples_piano[0], 3000, 'PCM_24')
    sf.write('pred.wav', piano_transformed[0], 3000, 'PCM_24')
    sf.write('target.wav', samples_guitar[0], 3000, 'PCM_24')
    ...
    print('Valid Loss:',valid_loss / (len(dataloader_test) / batch_size))
