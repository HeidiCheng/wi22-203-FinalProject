...
if not args.f:  # Not single file prediction
    ...
    for samples_piano, samples_guitar in dataloader_test:
        ...
        with torch.no_grad():
            ...
            for k in range(batch_size):
                torchaudio.save(os.path.join(args.out, str(sample_num) + '_input_pitch' + '.wav'), samples_piano[k], sample_rate, format='wav')
                torchaudio.save(os.path.join(args.out, str(sample_num) + '_pred_pitch' + '.wav'), piano_transformed_shaped[k], sample_rate, format='wav')
                torchaudio.save(os.path.join(args.out, str(sample_num) + '_tgt_pitch' + '.wav'),samples_guitar[k], sample_rate, firmat='wav')
                sample_num += 1
    ...
    with torch.no_grad():
        ...
        torchaudio.save(os.path.join(args.out, 'input_pitch_' + fname), mono_piano_wav[0], sample_rate, format='wav')
        torchaudio.save(os.path.join(args.out, 'pred_pitch_' + fname), piano_transformed[0], sample_rate, format='wav')